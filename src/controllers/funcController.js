...
class funcionarioController {
  ...
  static async create(req, res) {                                  //informações para rotas
    ...
    if (
      ...
      const funcionarioExists = await funcionarioModels.findOne({ where: { cpf: cpf } })   //localizar informações já existentes
      ...
      try {                                                       //tratar erro
        ...
        res.status(200).json({ status: 200, message: 'Funcionário cadastrado com sucesso!' })
      } catch (error) {
        return res.status(400).json({ status: 400, message: error.message })
      }
    }
    ...
    static async updateById(req, res) {
      ...
      try {
        ...
        return res.status(200).json({ status: 200, message: 'Funcionário atualizado com sucesso!' })
      } catch (error) {
        return res.status(400).json({ status: 400, message: `Algo deu errado: ${error}` })
      }
  ...
  static async deleteById(req, res) {
    ...
    try {
      ...
      return res.status(200).json({status: 200, message: 'Funcionário deletado!'})
    } catch (error) {
